...
func TestBinancePerp_FetchOHLCVs(t *testing.T) {
	...
	ohlcvs, err := perp.FetchOHLCVs(ctx, symbol, timeframe, since, limit)
	if err != nil {
...
func TestBinancePerp_FetchMarkets(t *testing.T) {
	// 创建 Binance 实例（从环境变量获取配置）
	ex, err := setupTestExchange()
	if err != nil {
		t.Fatalf("Failed to create Binance instance: %v", err)
...
func TestBinancePerp_FetchTickers(t *testing.T) {
	// 创建 Binance 实例（从环境变量获取配置）
	ex, err := setupTestExchange()
	if err != nil {
		t.Fatalf("Failed to create Binance instance: %v", err)
	...
	// 测试 FetchTickers
	tickers, err := perp.FetchTickers(ctx)
	if err != nil {
		if shouldSkipTestForHTTPError(t, err) {
			return
		}
