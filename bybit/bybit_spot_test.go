...
func setupTestExchange() (exchange.Exchange, error) {
	...
	return NewBybit(apiKey, secretKey, options)
}
...
func TestBybitSpot_FetchOHLCVs(t *testing.T) {
...
func TestBybitSpot_FetchMarkets(t *testing.T) {
	// 创建 Bybit 实例（从环境变量获取配置）
	ex, err := setupTestExchange()
	if err != nil {
		t.Fatalf("Failed to create Bybit instance: %v", err)
...
func TestBybitSpot_FetchTickers(t *testing.T) {
	// 创建 Bybit 实例（从环境变量获取配置）
	ex, err := setupTestExchange()
	if err != nil {
		t.Fatalf("Failed to create Bybit instance: %v", err)
