...
func main() {
	...
	if apiKey == "" || secretKey == "" {
		log.Println("警告: 未设置 BINANCE_API_KEY 和 BINANCE_SECRET_KEY 环境变量")
	...
	params := map[string]interface{}{
		// Binance 单向模式下，buy 就是开多
		// 双向模式下可以使用: "positionSide": "LONG"
	}
	...
	fmt.Println("\n3. 合约卖出开空")
	params = map[string]interface{}{
		// Binance 单向模式下，sell 就是开空
		// 双向模式下可以使用: "positionSide": "SHORT"
	}
	order, err = exchange.CreateOrder(ctx, contractSymbol, types.OrderSideSell, types.OrderTypeMarket, amount, 0, params)
	if err != nil {
